...
public class WebSocketServerHandler2 extends ChannelHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (group.size() > 0) {
            ...
            if (listZL.size() > 0) {
                ...
                String weight = strs.substring(0, strs.length() - 2);
                b_weight = Double.valueOf(weight.toString());
                System.out.println(b_weight);
                if (inputStr.length() > 6) {
                    if ("ST NT +".equals(inputStr.substring(0, 7))) {
                        System.out.println("第二次称重");
                        listZL.removeAll(listZL);
                        listZL.add(inputStr.substring(7));
                    }else {
                        fenjian(b_weight,inputStr, port, c_id, order, o_orderid, echoContent);
                    }
                } else {
                    fenjian(b_weight,inputStr, port, c_id, order, o_orderid, echoContent);
                }
                ...
                if (listZL.size() == 0) {
                    System.out.println("还没有称重");
                    System.out.println("不进行任何处理!");
                    WebSocketServerHandler.channelGroup.writeAndFlush(new TextWebSocketFrame("weight"));
                }
            }
    ...
    public static void show3(String inputStr, Date date) throws Exception {
        ...
        String n_Item_info = "";
        Double n_weight = Double.valueOf(listZL.get(0));
    ...
    //分拣
    public static void fenjian(Double b_weight,String inputStr, String port, String c_id, String order, String o_orderid, String echoContent) throws Exception {
        //获取当前时间
        Date d = new Date();
